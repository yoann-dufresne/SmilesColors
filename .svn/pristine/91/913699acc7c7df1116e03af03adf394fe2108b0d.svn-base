...
Interface.prototype.pagesHandler = function () {
	model.peptides.sort(model.sortByCoverage);
	inter.createPages ("peptides", model.peptides);
	model.monomers.sort(model.sortByCoverage);
	inter.createPages ("monomers", model.monomers);
	model.others.sort(model.sortByCoverage);
...
Interface.prototype.printResults = function (id, coverages) {
	...
	while (divData.children.length > 0) 
		divData.removeChild(divData.children[0]);
...
Interface.prototype.statsHandler = function () {
	...
	if (div.children.length == 1) {
		...
		parameters.classList.add("pannels");
		div.appendChild(parameters);
		inter.createParametersStats(parameters);
	} else {
...
Interface.prototype.createProportionsStats = function (div) {
	...
	for (var i=0; i<segments.length; i++){
		...
		if(segments[i].label)
			item += "<span id='chartLabel_" + segments[i].label + "'></span>";
		item += "</li>";
		legend.innerHTML += item;
	}
	div.appendChild(legend);

	this.updateProportionsStats ();
}
...
Interface.prototype.updateProportionsStats = function () {
	...
	var label = document.getElementById("chartLabel_" + chart.segments[0].label);
	label.innerHTML = chart.segments[0].label + " : " + model.peptides.length;

	chart.segments[1].value = model.monomers.length;
	label = document.getElementById("chartLabel_" + chart.segments[1].label);
	label.innerHTML = chart.segments[1].label + " : " + model.monomers.length;

	chart.segments[2].value = model.others.length;
	label = document.getElementById("chartLabel_" + chart.segments[2].label);
	label.innerHTML = chart.segments[2].label + " : " + model.others.length;
...
Interface.prototype.createParametersStats = function (div) {
	var label = document.createElement("p");
	label.innerHTML = "Caracteristics to be a peptide :";
	div.appendChild (label);

	label = document.createElement("label");
	label.innerHTML = "Coverage ratio";
	div.appendChild (label);
	...
	inter.ratioPeps = ratio;


	label = document.createElement("label");
	label.innerHTML = "Monomer number";
	div.appendChild (label);

	var nbMonos = document.createElement("input");
	nbMonos.type = "text";
	nbMonos.value = "2";
	div.appendChild(nbMonos);
	inter.nbMonosPeps = nbMonos;



	label = document.createElement("p");
	label.innerHTML = "Caracteristics to be a monomer :";
	div.appendChild (label);
	...
	button.onclick = function () {
		loading.ratioMonos = inter.ratioMonos.value;
		loading.ratioPeps = inter.ratioPeps.value;
		loading.nbMatchsPep = inter.nbMonosPeps.value;
